/**
 * @file libdcmpf_diag_reset_p.h
 * @copyright Copyright (c) 2022 TOYOTA MOTOR CORPORATION All Rights Reserved.
 * 
 * @brief \~japanese ECUリセット機能モジュール(プロトタイプ宣言)
 * @brief \~english ECU Reset feature module (prototype definition)
 * 
 */
#ifndef LIBDCMPF_DIAG_RESET_P_H
#define LIBDCMPF_DIAG_RESET_P_H

#include <dcmpf/common/libdcmpf_type.h>
#include <dcmpf/diag/reset/libdcmpf_diag_reset_c.h>
#include <dcmpf/diag/reset/libdcmpf_diag_reset_s.h>
 ...
 * @n
 * 登録を解除する場合は、 dcmpf_diag_reset_unregister_check_precondition() を呼び出し解除する。 @n
 ...
 * @n
 * If it want to cancel the registration, you can cancel the call to dcmpf_diag_reset_unregister_check_precondition() . @n
 ...
 * 
 */
extern dcmpf_api_status_t dcmpf_diag_reset_register_check_precondition(
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    引数 handle がNULLもしくは不正の場合 @n
 *                                           引数 callback がNULLの場合
 * @retval DCMPF_API_STATUS_NO_SUCH          @ref dcmpf_diag_reset_register_check_precondition() によりコールバック関数が登録されていない場合
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    If the argument handle is NULL or invalid
 *                                           If the argument callback is NULL
 * @retval DCMPF_API_STATUS_NO_SUCH          If the callback function is not registered using @ref dcmpf_diag_reset_register_check_precondition()
 * @retval DCMPF_API_STATUS_FAILED           If the unregistration fails
 *
 * @details
 * Unregister the precondition check function. @n
 * @n
 * If this API is called when it is already unregistered, it returns @ref DCMPF_API_STATUS_SUCCESS . @n
 * @n
 * 
 */
extern dcmpf_api_status_t dcmpf_diag_reset_unregister_check_precondition(
    dcmpf_handle_t handle,
    dcmpf_diag_reset_request_check_precondition_cb_t callback
);
 ...
 * 
 */
extern dcmpf_api_status_t dcmpf_diag_reset_report_check_precondition(
    dcmpf_handle_t handle,
    uint8_t order_no,

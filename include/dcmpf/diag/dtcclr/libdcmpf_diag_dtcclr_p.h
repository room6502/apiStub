/**
 * @file libdcmpf_diag_dtcclr_p.h
 * @copyright Copyright (c) 2022 TOYOTA MOTOR CORPORATION All Rights Reserved.
 *
 * @brief \~japanese DTCクリア機能モジュール(プロトタイプ宣言)
 * @brief \~english DTC Clear feature module (prototype definition)
 *
 */
#ifndef LIBDCMPF_DIAG_DTCCLR_P_H
#define LIBDCMPF_DIAG_DTCCLR_P_H

#include <dcmpf/common/libdcmpf_type.h>
#include <dcmpf/diag/dtcclr/libdcmpf_diag_dtcclr_c.h>
#include <dcmpf/diag/dtcclr/libdcmpf_diag_dtcclr_s.h>
 ...
 * @n
 * 登録を解除する場合は、 dcmpf_diag_dtcclr_unregister_check_precondition() を呼び出し解除する。 @n
 ...
 * @n
 * If it want to cancel the registration, you can cancel the call to dcmpf_diag_dtcclr_unregister_check_precondition() . @n
 ...
 *
 */
extern dcmpf_api_status_t dcmpf_diag_dtcclr_register_check_precondition(
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    引数 handle がNULLもしくは不正の場合 @n
 *                                           引数 callback がNULLの場合
 * @retval DCMPF_API_STATUS_NO_SUCH          @ref dcmpf_diag_dtcclr_register_check_precondition() によりコールバック関数が登録されていない場合
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    If the argument handle is NULL or invalid
 *                                           If the argument callback is NULL
 * @retval DCMPF_API_STATUS_NO_SUCH          If the callback function is not registered using @ref dcmpf_diag_dtcclr_register_check_precondition()
...
*
 */
extern dcmpf_api_status_t dcmpf_diag_dtcclr_unregister_check_precondition(
    dcmpf_handle_t handle,
    dcmpf_diag_dtcclr_request_check_precondition_cb_t callback
);
 ...
 *
 */
extern dcmpf_api_status_t dcmpf_diag_dtcclr_report_check_precondition(
    dcmpf_handle_t handle,
    uint8_t order_no,
...
/**
 * @brief \~japanese DTC消去完了通知コールバック関数を開始する
 *
 * @param [in] handle       @ref dcmpf_diag_control_context_create() で生成したコンテキストハンドル
 ...
 *
 * @details
 * DTC消去完了通知コールバック関数を開始する。 @n
 * @n
 * 通知開始後、DTCが消去されるとコールバック関数が呼び出される。 @n
 * 起動後、このAPIが呼び出されるまでの間、通知は行われない。 @n
 * 通知を停止する場合は、 dcmpf_diag_dtcclr_stop_complete_notify() を呼び出し通知を停止する。 @n
 * 既に指定されたコールバック関数への通知が開始している状態でこのAPIを呼び出した場合 @ref DCMPF_API_STATUS_SUCCESS を返す。 @n
 *
 *
 * @brief \~english Start the DTC clear notification callback function
 *
 * @param [in] handle       Context handle created by @ref dcmpf_diag_control_context_create()
 ...
 *
 * @details
 * Start the DTC clear notification callback function. @n
 * @n
 * After the notification starts, the callback function is called every time the DTC cleares. @n
 * After startup, the change notification is not performed until this API is called. @n
 * To stop the notification, call dcmpf_diag_dtcclr_stop_complete_notify() to stop the notification. @n
 * If this API is called while notification to the specified callback function has already started, @ref DCMPF_API_STATUS_SUCCESS is returned. @n
 *
 */
extern dcmpf_api_status_t dcmpf_diag_dtcclr_start_complete_notify(
    dcmpf_handle_t handle,
    dcmpf_diag_dtcclr_notify_complete_cb_t callback
);

/**
 * @brief \~japanese DTC消去完了通知コールバック関数を停止する
 *
 * @param [in] handle       @ref dcmpf_diag_control_context_create() で生成したコンテキストハンドル
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    引数 handle がNULLもしくは不正の場合 @n
 *                                           引数 callback がNULLの場合
 * @retval DCMPF_API_STATUS_NO_SUCH          dcmpf_diag_dtcclr_start_complete_notify() により、コールバック関数が登録されていない場合
 * @retval DCMPF_API_STATUS_FAILED           停止に失敗した場合
 *
 * @details
 * DTC消去完了通知コールバック関数を停止する。 @n
 * @n
 * 登録済みコールバック関数の解除を行う。 @n
 *
 *
 * @brief \~english Stop the DTC clear notification callback function
 *
 * @param [in] handle       Context handle created by @ref dcmpf_diag_control_context_create()
 * @param [in] callback     Callback function pointer to notify the result @n
 *                          As described in [ "About function pointer encryption" ]( @ref guideline_caution_encode_pointer ),
 ...
 * @retval DCMPF_API_STATUS_INVALID_PARAM    If the argument handle is NULL or invalid @n
 *                                           If the argument callback is NULL @n
 * @retval DCMPF_API_STATUS_NO_SUCH          If the callback function is not registered using dcmpf_diag_dtcclr_start_complete_notify()
 * @retval DCMPF_API_STATUS_FAILED           If it fails to stop
 *
 * @details
 * Stop the DTC clear notification callback function. @n
 * @n
 * The registered callback functions are unregistered. @n
 *
 *
 */
extern dcmpf_api_status_t dcmpf_diag_dtcclr_stop_complete_notify(
    dcmpf_handle_t handle,
    dcmpf_diag_dtcclr_notify_complete_cb_t callback
);
...
/** @} */
/** @} */
#endif	/* LIBDCMPF_DIAG_DTCCLR_P_H */
